...
class MT5:
   ...
   def get_data(self, symbol, n, timeframe=mt5.TIMEFRAME_D1):
   ...
   def orders(self, symbol, lot, buy=True, id_position=None):
       """ Send the orders """
   ...
   def run(self, symbol, long, short, lot):
        ...
        if long==True and position==0:
            ...
            res = self.orders(symbol, lot, buy=True, id_position=identifier)
            ...
            res = self.orders(symbol, lot, buy=False, id_position=identifier)
        ...
        """ Buy or short """
        if long==True:
            res = self.orders(symbol=symbol, lot=lot, buy=True)
            print(f"OPEN LONG TRADE: {res}")

        if short==True:
            res = self.orders(symbol=symbol, lot=lot, buy=False)
            print(f"OPEN SHORT TRADE: {res}")

        print("------------------------------------------------------------------")

   def close_all_night(self):
        result = self.resume()
        for i in range(len(result)):
            before =  mt5.account_info().balance
            row = result.iloc[0+i:1+i,:]
            if row["position"][0]==0:
                res = self.orders(symbol=row["symbol"][0], lot=row["volume"][0], buy=True, id_position=row["ticket"][0])

            else:
                res = self.orders(symbol=row["symbol"][0], lot=row["volume"][0], buy=False, id_position=row["ticket"][0])